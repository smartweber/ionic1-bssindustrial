(function() {
	...
	DrupalArticleController.$inject = [
		'$stateParams', '$ionicActionSheet', '$cordovaSocialSharing', 'drupalService', 'motion', '$timeout', 'KeysConsts'];

	/* @ngInject */
	function DrupalArticleController(
		$stateParams, $ionicActionSheet, $cordovaSocialSharing, drupalService, motion, $timeout, KeysConsts) {
		...
		if(window.cordova) {
			...
			$timeout(function(){
				AdMob.createBanner( {
					...
					autoShow: true,
					bgColor: 'black',
					position: AMob.AD_POSITION.BOTTOM_CENTER
				});
			}, 20000);
		}			
